...
/**
 * Advanced helper to extract state from a complaint object.
 * Checks for explicit 'state' field first, then falls back to parsing the address.
 */
export const extractState = (complaint: any) => {
    if (complaint.state) return complaint.state;
    if (!complaint.location?.address) return 'Unknown';

    const address = complaint.location.address;
    const parts = address.split(',').map((p: string) => p.trim());

    if (parts.length >= 2) {
        // Indian addresses often end with State/Region or Pincode
        const lastPart = parts[parts.length - 1];

        // If last part is a pincode (digits), the state is usually the second-to-last part
        const isPincode = /^\d+$/.test(lastPart);
        if (isPincode && parts.length >= 3) return parts[parts.length - 2];
